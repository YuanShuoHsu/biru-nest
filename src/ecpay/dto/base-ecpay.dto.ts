...
export class BaseEcpayDto {
  ...
  @ApiProperty({
    description: `商品名稱（必填）
    <ul>
    <li>如果商品名稱有多筆，需在金流選擇頁一行一行顯示商品名稱的話，商品名稱請以符號 # 分隔。</li>
  ...
  @ApiProperty({
    ...
    <ul>
    <li>ReturnURL 為付款結果通知回傳網址，為特店 server 或主機的 URL，用來接收綠界後端回傳的付款結果通知。</li>
    <li>當消費者付款完成後，綠界會將付款結果參數以幕後回傳到該網址。詳細說明請參考付款結果通知</li>
    </ul>
    注意事項：
    <ol>
    <li>請勿設定與 Client 端接收付款結果網址 OrderResultURL 相同位置，避免程式判斷錯誤。</li>
    <li>ReturnURL 收到綠界後端回傳的付款結果通知後，請回應字串 1|OK 給綠界。</li>
    <li>1|OK 僅是廠商回應綠界是否收到通知，並不會改變付款狀態。</li>
  ...
  @ApiProperty({
    description: `選擇預設付款方式（必填）<br>
    綠界提供下列付款方式：
    <ul>
    <li>Credit：信用卡及銀聯卡（需申請開通）</li>
    <li>TWQR：歐付寶 TWQR 行動支付（需申請開通）</li>
    <li>WebATM：網路ATM</li>
    <li>ATM：自動櫃員機</li>
    <li>CVS：超商代碼</li>
    <li>BARCODE：超商條碼</li>
    <li>ApplePay：Apple Pay（僅支援手機支付）</li>
    <li>BNPL：裕富無卡分期（需申請開通）</li>
    <li>WeiXin：微信支付</li>
    <li>ALL：不指定付款方式，由綠界顯示付款方式選擇頁面。</li>
    </ul>
    注意事項：
    <ol>
    <li>若為手機版時不支援下列付款方式：WebATM</li>
    <li>如需要不透過綠界畫面取得 ATM、CVS、BARCODE 的繳費代碼，請參考如何不經過綠界畫面取得 ATM、CVS、BARCODE 的繳費代碼。</li>
    <li>當瀏覽器不為 Safari 時，不會顯示 Apple Pay 付款功能。</li>
    </ol>`,
    enum: BaseEcpayChoosePayment,
  ...
  @ApiProperty({
    description: `CheckMacValue 加密類型（必填）<br>
  ...
  @ApiPropertyOptional({
    description: `Client 端返回特店的按鈕連結<br>
    消費者點選此按鈕後，會將頁面導回到此設定的網址<br>
    注意事項：
    <ol>
    <li>導回時不會帶付款結果到此網址，只是將頁面導回而已。</li>
    <li>設定此參數，綠界會在付款完成或取號完成頁面上顯示 [返回商店] 的按鈕。</li>
    <li>設定此參數，發生簡訊 OTP 驗證失敗時，頁面上會顯示 [返回商店] 的按鈕。</li>
    <li>若未設定此參數，則綠界付款完成頁或取號完成頁面，不會顯示 [返回商店] 的按鈕。</li>
    <li>若導回網址未使用 https 時，部份瀏覽器可能會出現警告訊息。</li>
    <li>參數內容若有包含 %26(&) 及 %3C(<) 這二個值時，請先進行 urldecode() 避免呼叫 API 失敗。</li>
    </ol>`,
  ...
  @ApiPropertyOptional({
    description: `Client 端回傳付款結果網址<br>
    有別於 ReturnURL（server端的網址），OrderResultURL 為特店的 client 端（前端）網址。消費者付款完成後，綠界會將付款結果參數以 POST 方式回傳到到該網址。詳細說明請參考付款結果通知。<br>
    注意事項：
    <ol>
    <li>若與 [ClientBackURL] 同時設定，將會以此參數為主。</li>
    <li>銀聯卡及非即時交易（ATM、CVS、BARCODE）不支援此參數。</li>
    <li>付款結果通知請依 ReturnURL（server端的網址）為主,避免因前端操作或網路問題未收到 OrderResultURL 特店的 client 端（前端）的通知。</li>
    <li>參數內容若有包含 %26(&) 及 %3C(<) 這二個值時，請先進行 urldecode() 避免呼叫 API 失敗。</li>
    </ol>`,
  ...
  @ApiPropertyOptional({
    description: `是否需要額外的付款資訊<br>
    額外的付款資訊
    <ul>
    <li>若不回傳額外的付款資訊時，參數值請傳：Ｎ；</li>
    <li>若要回傳額外的付款資訊時，參數值請傳：Ｙ ，付款完成後綠界後端會以 POST 方式回傳額外付款資訊到特店的 ReturnURL 與 OrderResultURL。</li>
    </ul>
  ...
  @ApiPropertyOptional({
    description: `隱藏付款方式<br>
    當付款方式 [ChoosePayment] 為 ALL 時，可隱藏不需要的付款方式，多筆請以井號分隔 (#)。<br>
    可用的參數值：
    <ul>
    <li>Credit：信用卡</li>
    <li>ApplePay：Apple Pay</li>
    <li>WebATM：網路ATM</li>
    <li>ATM：自動櫃員機</li>
    <li>CVS：超商代碼</li>
    <li>BARCODE：超商條碼</li>
